...
function lf_request_fss_approval($row, $data) {
    ...
    $subject = 'Kappingarloyvi: ' . trim(implode(' ', $subject_parts));

    $link = add_query_arg('lf_fss_approve', rawurlencode($fss_token), get_site_url());

    $body  = "Ein umsókn um kappingarloyvi er nú klár til endaliga góðkenning frá FSS.\n\n";
    $body .= "Navn: {$name}\nFelag: {$club}\nTeldupostur: {$email}\n\n";
    $body .= "Góðkenn her:\n{$link}\n";
...
function lf_admin_send_club_approval_link($row, $data) {
    ...
    $body  = "Ein umsókn um kappingarloyvi krevur góðkenning frá felagnum.\n\n";
    $body .= "Navn: {$name}\n";
    $body .= "Felag: {$club}\n";
...
function lf_admin_send_guardian_approval_link($row, $data) {
    ...
    $body .= "Ein umsókn um kappingarloyvi krevur tína góðkenning sum verji.\n\n";
...
function lf_send_denial_notifications($data, $role_label, $denied_by, $reason) {
    ...
    if ($name) $subject_parts[] = $name;
    if ($club) $subject_parts[] = '(' . $club . ')';
    $subject = 'Kappingarloyvi NOKTAÐ: ' . trim(implode(' ', $subject_parts));

    $body  = "Ein umsókn um kappingarloyvi er NOKTAÐ.\n\n";
    $body .= "Umsókn: {$name}\n";
    $body .= "Felag: {$club}\n";
...
function lf_finalize_approval($row, $data) {
    ...
    $body  = "Nýtt kappingarloyvi er móttiki og nú fullgóðkent:\n\n";
...
function lf_handle_approval() {
    ...
    if (!$row) {
        wp_die('<p>Umsókn fannst ikki ella er ikki longur virkandi.</p>', 'Kappingarloyvi', ['response' => 200]);
    }

    if ($row->status === 'approved') {
        wp_die('<p>Henda umsókn er longu endaliga góðkend og send.</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    if ($row->status === 'denied') {
        $d = maybe_unserialize($row->data);
        $reason = is_array($d) ? ($d['denied_reason'] ?? '') : '';
        wp_die('<p>Henda umsókn er noktað.</p>' . ($reason ? '<p><strong>Orsøk:</strong> ' . esc_html($reason) . '</p>' : ''), 'Kappingarloyvi', ['response' => 200]);
    }

    $data = maybe_unserialize($row->data);
    if (!is_array($data)) {
        wp_die('<p>Umsóknarkanning miseydnaðist.</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['lf_approved_name']) || isset($_POST['lf_deny']))) {
        ...
        if (isset($_POST['lf_deny'])) {
            ...
            if ($deny_name === '') {
                wp_die('<p>Vinaliga skriva navn.</p>', 'Kappingarloyvi', ['response' => 200]);
            }
            if ($deny_reason === '') {
                wp_die('<p>Vinaliga skriva eina viðmerking um, hví umsóknin verður noktað.</p>', 'Kappingarloyvi', ['response' => 200]);
            }
            lf_mark_denied($row, $data, 'Felag', $deny_name, $deny_reason);
            wp_die('<p>Umsóknin er noktað. Allir partar hava fingið teldupost um tað.</p>', 'Kappingarloyvi', ['response' => 200]);
        }

        // Approve flow
        $approved_by = sanitize_text_field(wp_unslash($_POST['lf_approved_name'] ?? ''));
        if ($approved_by === '') {
            wp_die('<p>Vinaliga skriva navn á tann, sum góðkennir umsóknina.</p>', 'Kappingarloyvi', ['response' => 200]);
        }
        ...
        $form_html .= '<h2>Góðkenning av kappingarloyvi (felag)</h2>';
        ...
        $form_html .= '<textarea name="lf_deny_reason" rows="3" style="width:100%;padding:0.5rem;" placeholder="Skriva hví kappingarloyvið verður nokta..."></textarea></p>';
        $form_html .= '<p style="display:flex;gap:0.75rem;flex-wrap:wrap;">'
            . '<button type="submit" style="padding:0.5rem 1.2rem;">Góðkenn</button>'
            . '<button type="submit" name="lf_deny" value="1" onclick="var t=this.form.querySelector(\'textarea[name=\\\"lf_deny_reason\\\"]\'); if(!t||!t.value.trim()){alert(\'Skriva viðmerking um hví umsóknin verður noktað.\'); return false;} return confirm(\'Ert tú viss(ur) í, at tú vilt nokta kappingarloyvið?\');" style="padding:0.5rem 1.2rem;background:#b71c1c;color:#fff;border:none;border-radius:4px;">Nokta</button>'
            . '</p>';
        $form_html .= '</form>';

        wp_die($form_html, 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    if ($finalized) {
        wp_die('<p>Takk! Tú hevur góðkent umsóknina. Allir kravdir partar hava nú góðkent, og endaligu teldupostarnir eru sendir við PDF-skjalinum.</p>', 'Kappingarloyvi', ['response' => 200]);
    }

    wp_die('<p>Takk! Tú hevur góðkent umsóknina vegna felagnum. Umsóknin bíðar nú eftir hinum góðkenningunum.</p>', 'Kappingarloyvi', ['response' => 200]);
}
...
function lf_handle_guardian_approval() {
    ...
    if (!$row) {
        wp_die('<p>Umsókn fannst ikki ella er ikki longur virkandi (verji).</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    if (!$is_minor) {
        wp_die('<p>Henda umsókn krevur ikki góðkenning frá verjanum.</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['lf_approved_name']) || isset($_POST['lf_deny']))) {
        ...
        if ($guardian_approved_by === '') {
            wp_die('<p>Vinaliga skriva navn á verjan, sum góðkennir umsóknina.</p>', 'Kappingarloyvi', ['response' => 200]);
        }
        ...
        $form_html .= '<h2>Góðkenning av kappingarloyvi (verji)</h2>';
        $form_html .= $status_html;
        $form_html .= lf_build_application_summary_html($data);
    ...
    if ($finalized) {
        wp_die('<p>Takk! Tú hevur góðkent umsóknina sum verji. Allir kravdir partar hava nú góðkent, og endaligu teldupostarnir eru sendir við PDF-skjalinum.</p>', 'Kappingarloyvi', ['response' => 200]);
    }

    wp_die('<p>Takk! Tú hevur góðkent umsóknina sum verji. Umsóknin bíðar nú eftir hinum góðkenningunum.</p>', 'Kappingarloyvi', ['response' => 200]);
}
...
function lf_handle_fss_approval() {
    ...
    if (!$row) {
        wp_die('<p>Umsókn fannst ikki ella er ikki longur virkandi (FSS).</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    $name = $data['name'] ?? '';
    $deny_default_reason = "{$name} hevur ikki eitt skrásett ADD skeið í ADD skipanini hjá FSS, og verður tí biðin um at fullfíggja tað áðrenn hon er játtað kappingarloyvið.\n\n";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['lf_approved_name']) || isset($_POST['lf_deny']))) {
        ...
        if (isset($_POST['lf_deny'])) {
            ...
            wp_die('<p>Umsóknin er noktað. Allir partar hafa fingið teldupost um tað.</p>', 'Kappingarloyvi', ['response' => 200]);
        }

        // Approve flow
        $fss_approved_by = sanitize_text_field(wp_unslash($_POST['lf_approved_name'] ?? ''));
        if ($fss_approved_by === '') {
            wp_die('<p>Vinaliga skriva navn á tann, sum góðkennir í FSS.</p>', 'Kappingarloyvi', ['response' => 200]);
        }
        ...
        if ($finalized) {
            wp_die('<p>Takk! FSS hevur góðkent. Allir kravdir partar hava nú góðkent, og endaligu teldupostarnir eru sendir við PDF-skjalinum.</p>', 'Kappingarloyvi', ['response' => 200]);
        }

        wp_die('<p>Takk! FSS hevur góðkent. Umsóknin bíðar nú eftir hinum góðkenningunum.</p>', 'Kappingarloyvi', ['response' => 200]);
    }
    ...
    $form_html .= '<p><label>Navn<br><input type="text" name="lf_approved_name" required style="width:100%;padding:0.5rem;"></label></p>';
    $form_html .= '<p style="margin-top:0.5rem;"><label style="font-weight:600;">Nokta við viðmerking</label><br>';
    $form_html .= '<textarea name="lf_deny_reason" rows="6" style="width:100%;padding:0.5rem;" placeholder="Skriva hví kappingarloyvið verður nokta...">' . esc_textarea($deny_default_reason) . '</textarea></p>';
    $form_html .= '<p style="display:flex;gap:0.75rem;flex-wrap:wrap;">'
        . '<button type="submit" style="padding:0.5rem 1.2rem;">Góðkenn</button>'
        . '<button type="submit" name="lf_deny" value="1" onclick="var t=this.form.querySelector(\'textarea[name=\\\"lf_deny_reason\\\"]\'); if(!t||!t.value.trim()){alert(\'Skriva viðmerking um hví umsóknin verður noktað.\'); return false;} return confirm(\'Ert tú viss(ur) í, at tú vilt nokta kappingarloyvið?\');" style="padding:0.5rem 1.2rem;background:#b71c1c;color:#fff;border:none;border-radius:4px;">Nokta</button>'
        . '</p>';
